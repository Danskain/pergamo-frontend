import { DatePipe } from '@angular/common';
import { HttpErrorResponse } from '@angular/common/http';
import {
  ChangeDetectionStrategy,
  Component,
  computed,
  effect,
  inject,
  signal
} from '@angular/core';
import { rxResource } from '@angular/core/rxjs-interop';
import { FormBuilder, ReactiveFormsModule, Validators } from '@angular/forms';
import { firstValueFrom, of } from 'rxjs';
import {
  NbButtonModule,
  NbIconModule,
  NbInputModule,
  NbSelectModule,
  NbSortDirection,
  NbSortRequest,
  NbToastrService,
  NbToggleModule,
  NbTreeGridDataSource,
  NbTreeGridDataSourceBuilder,
  NbTreeGridModule
} from '@nebular/theme';

import { CreateAccountingGroupUseCase } from '../../../application/use-cases/create-accounting-group.use-case';
import { DeleteAccountingGroupUseCase } from '../../../application/use-cases/delete-accounting-group.use-case';
import { GetAccountingGroupDetailUseCase } from '../../../application/use-cases/get-accounting-group-detail.use-case';
import { ListAccountingGroupsUseCase } from '../../../application/use-cases/list-accounting-groups.use-case';
import { RestoreAccountingGroupUseCase } from '../../../application/use-cases/restore-accounting-group.use-case';
import { UpdateAccountingGroupUseCase } from '../../../application/use-cases/update-accounting-group.use-case';
import { SelectOption } from '../../../domain/models/accounting-select-option.model';
import { injectAccountingCatalogOptions } from '../../form-support/inject-accounting-catalog-options';
import {
  AccountingGroup,
  AccountingGroupPage,
  AccountingGroupPayload
} from '../../../domain/models/accounting-group.model';
import { CrudCollectionShellComponent } from '../../../../../shared/presentation/components/crud-collection-shell/crud-collection-shell.component';
import { applyCrudColumnPolicy } from '../../../../../shared/presentation/components/crud-column-policy';
import { CrudPageHeroComponent } from '../../../../../shared/presentation/components/crud-page-hero/crud-page-hero.component';
import { CrudSectionCardComponent } from '../../../../../shared/presentation/components/crud-section-card/crud-section-card.component';

type FeedbackStatus = 'success' | 'danger' | 'info';

type TreeNode<T> = {
  data: T;
  children?: TreeNode<T>[];
  expanded?: boolean;
};

type AccountingGroupGridRow = {
  id: number;
  code: string;
  name: string;
  description: string;
  accountClassName: string;
  accountFrom: number;
  accountTo: number;
  affectsClosing: boolean;
  affectsFinancialStatements: boolean;
  createdAt: string;
  updatedAt: string;
  deletedAt: string | null;
};

@Component({
  selector: 'app-grupo-contable-page',
  imports: [
    DatePipe,
    ReactiveFormsModule,
    NbButtonModule,
    NbIconModule,
    NbInputModule,
    NbSelectModule,
    NbToggleModule,
    NbTreeGridModule,
    CrudCollectionShellComponent,
    CrudPageHeroComponent,
    CrudSectionCardComponent
  ],
  templateUrl: './grupo-contable-page.component.html',
  styleUrl: './grupo-contable-page.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class GrupoContablePageComponent {
  private static readonly FORM_CATALOGS = ['account_class'];

  private readonly formBuilder = inject(FormBuilder);
  private readonly toastrService = inject(NbToastrService) as {
    show: (
      message: string,
      title: string,
      config: {
        status: FeedbackStatus;
        duration: number;
        destroyByClick: boolean;
        hasIcon: boolean;
      }
    ) => void;
  };
  private readonly treeGridDataSourceBuilder = inject(
    NbTreeGridDataSourceBuilder
  ) as NbTreeGridDataSourceBuilder<AccountingGroupGridRow>;
  private readonly listAccountingGroupsUseCase = inject(ListAccountingGroupsUseCase);
  private readonly getAccountingGroupDetailUseCase = inject(GetAccountingGroupDetailUseCase);
  private readonly createAccountingGroupUseCase = inject(CreateAccountingGroupUseCase);
  private readonly updateAccountingGroupUseCase = inject(UpdateAccountingGroupUseCase);
  private readonly deleteAccountingGroupUseCase = inject(DeleteAccountingGroupUseCase);
  private readonly restoreAccountingGroupUseCase = inject(RestoreAccountingGroupUseCase);
  private readonly accountingCatalogOptions = injectAccountingCatalogOptions(
    GrupoContablePageComponent.FORM_CATALOGS,
    {
      limit: 100
    }
  );

  private readonly refreshTick = signal(0);
  private readonly detailRefreshTick = signal(0);
  protected readonly page = signal(1);
  protected readonly perPage = signal(28);
  protected readonly editingId = signal<number | null>(null);
  protected readonly showForm = signal(false);
  protected readonly submitting = signal(false);
  protected readonly deletingId = signal<number | null>(null);
  protected readonly restoringId = signal<number | null>(null);
  protected readonly sortDirection = signal<NbSortDirection>(NbSortDirection.NONE);
  protected readonly sortColumn = signal('');
  protected readonly treeGridCustomColumn = 'name';
  protected readonly treeGridColumns = [
    'id',
    'code',
    'description',
    'accountClassName',
    'accountFrom',
    'accountTo',
    'affectsClosing',
    'affectsFinancialStatements',
    'createdAt',
    'actions'
  ];
  protected readonly treeGridAllColumns = [
    this.treeGridCustomColumn,
    ...applyCrudColumnPolicy(this.treeGridColumns)
  ];
  protected readonly treeGridDataSource = signal<
    NbTreeGridDataSource<AccountingGroupGridRow>
  >(this.treeGridDataSourceBuilder.create([] as TreeNode<AccountingGroupGridRow>[]));

  protected readonly form = this.formBuilder.group({
    code: ['', [Validators.required, Validators.maxLength(255)]],
    accountClassId: [null as number | null, [Validators.required, Validators.min(1)]],
    name: ['', [Validators.required, Validators.maxLength(255)]],
    description: ['', [Validators.required]],
    accountFrom: [null as number | null, [Validators.required, Validators.min(0)]],
    accountTo: [null as number | null, [Validators.required, Validators.min(0)]],
    affectsClosing: [false],
    affectsFinancialStatements: [false]
  });

  protected readonly accountingGroupsResource = rxResource<
    AccountingGroupPage,
    { page: number; perPage: number; refreshTick: number }
  >({
    params: () => ({
      page: this.page(),
      perPage: this.perPage(),
      refreshTick: this.refreshTick()
    }),
    stream: ({ params }) =>
      this.listAccountingGroupsUseCase.execute(params.page, params.perPage)
  });

  protected readonly selectedAccountingGroupResource = rxResource<
    AccountingGroup | null,
    { id: number | null; refreshTick: number }
  >({
    params: () => ({
      id: this.editingId(),
      refreshTick: this.detailRefreshTick()
    }),
    stream: ({ params }) =>
      params.id ? this.getAccountingGroupDetailUseCase.execute(params.id) : of(null),
    defaultValue: null
  });
  protected readonly selectOptionsResource = this.accountingCatalogOptions.resource;

  protected readonly accountingGroupPage = computed<AccountingGroupPage | null>(
    () => this.accountingGroupsResource.value() ?? null
  );
  protected readonly accountingGroups = computed<AccountingGroup[]>(
    () => this.accountingGroupPage()?.items ?? []
  );
  protected readonly pagination = computed(() => {
    const page = this.accountingGroupPage();

    return (
      page?.meta ?? {
        currentPage: 1,
        from: null,
        lastPage: 1,
        path: '',
        perPage: this.perPage(),
        to: null,
        total: 0
      }
    );
  });
  protected readonly accountClassOptions: () => Array<SelectOption<Record<string, unknown>>> =
    this.accountingCatalogOptions.optionsFor('account_class');
  protected readonly isEditing = computed(() => this.editingId() !== null);
  protected readonly formTitle = computed(() =>
    this.isEditing() ? 'Editar grupo contable' : 'Nuevo grupo contable'
  );
  protected readonly submitLabel = computed(() =>
    this.isEditing() ? 'Actualizar registro' : 'Crear registro'
  );
  protected readonly pageSummary = computed(() => {
    const pagination = this.pagination();

    if (pagination.total === 0) {
      return 'No hay registros para mostrar.';
    }

    return `Mostrando ${pagination.from ?? 0}-${pagination.to ?? 0} de ${pagination.total} registros`;
  });
  protected readonly canGoPrevious = computed(
    () => this.pagination().currentPage > 1
  );
  protected readonly canGoNext = computed(
    () => this.pagination().currentPage < this.pagination().lastPage
  );
  protected readonly perPageOptions = computed(() =>
    Array.from(new Set([10, 20, 28, 50, 100, this.pagination().perPage])).sort(
      (left, right) => left - right
    )
  );

  constructor() {
    effect(() => {
      const accountingGroup = this.selectedAccountingGroupResource.value();

      if (!accountingGroup) {
        return;
      }

      this.patchForm(accountingGroup);
    });

    effect(() => {
      const nodes = this.accountingGroups().map((accountingGroup) =>
        this.mapAccountingGroupToTreeNode(accountingGroup)
      );
      this.treeGridDataSource.set(this.treeGridDataSourceBuilder.create(nodes));
    });
  }

  protected startCreate(): void {
    this.showForm.set(true);
    this.editingId.set(null);
    this.detailRefreshTick.set(0);
    this.resetForm();
  }

  protected editAccountingGroup(id: number): void {
    this.showForm.set(true);
    this.showToast(
      'info',
      'Cargando detalle',
      'Se esta cargando la informacion completa del grupo contable seleccionado.'
    );
    this.editingId.set(id);
    this.detailRefreshTick.update((value) => value + 1);
  }

  protected async submit(): Promise<void> {
    if (this.form.invalid) {
      this.form.markAllAsTouched();
      this.showToast(
        'danger',
        'Formulario incompleto',
        'Revisa los campos obligatorios antes de guardar el grupo contable.'
      );
      return;
    }

    const payload = this.buildPayload();
    this.submitting.set(true);

    try {
      if (this.isEditing() && this.editingId()) {
        await firstValueFrom(
          this.updateAccountingGroupUseCase.execute(this.editingId()!, payload)
        );
        this.showToast(
          'success',
          'Registro actualizado',
          'El grupo contable se actualizo correctamente.'
        );
      } else {
        await firstValueFrom(this.createAccountingGroupUseCase.execute(payload));
        this.showToast(
          'success',
          'Registro creado',
          'El grupo contable se creo correctamente.'
        );
      }

      this.reloadList();
      this.closeForm();
    } catch (error) {
      this.showToast(
        'danger',
        'No fue posible guardar',
        this.extractErrorMessage(error)
      );
    } finally {
      this.submitting.set(false);
    }
  }

  protected async deleteAccountingGroup(id: number): Promise<void> {
    const confirmed = confirm(
      'Deseas eliminar este grupo contable? Esta accion no se puede deshacer.'
    );

    if (!confirmed) {
      return;
    }

    this.deletingId.set(id);

    try {
      await firstValueFrom(this.deleteAccountingGroupUseCase.execute(id));
      this.showToast(
        'success',
        'Registro eliminado',
        'El grupo contable fue eliminado correctamente.'
      );

      if (this.editingId() === id) {
        this.closeForm();
      }

      this.reloadList();
    } catch (error) {
      this.showToast(
        'danger',
        'No fue posible eliminar',
        this.extractErrorMessage(error)
      );
    } finally {
      this.deletingId.set(null);
    }
  }

  protected async restoreAccountingGroup(id: number): Promise<void> {
    const confirmed = confirm('Deseas restaurar este grupo contable eliminado?');

    if (!confirmed) {
      return;
    }

    this.restoringId.set(id);

    try {
      await firstValueFrom(this.restoreAccountingGroupUseCase.execute(id));
      this.showToast(
        'success',
        'Registro restaurado',
        'El grupo contable fue restaurado correctamente.'
      );
      this.reloadList();
    } catch (error) {
      this.showToast(
        'danger',
        'No fue posible restaurar',
        this.extractErrorMessage(error)
      );
    } finally {
      this.restoringId.set(null);
    }
  }

  protected refreshData(): void {
    this.reloadList();
    this.showToast(
      'info',
      'Datos actualizados',
      'Se recargo la informacion de grupos contables desde la API.'
    );
  }

  protected changeSort(sortRequest: NbSortRequest): void {
    this.sortColumn.set(sortRequest.column);
    this.sortDirection.set(sortRequest.direction);
  }

  protected getSortDirection(column: string): NbSortDirection {
    if (this.sortColumn() === column) {
      return this.sortDirection();
    }

    return NbSortDirection.NONE;
  }

  protected goToPreviousPage(): void {
    if (!this.canGoPrevious()) {
      return;
    }

    this.page.update((value) => value - 1);
  }

  protected goToNextPage(): void {
    if (!this.canGoNext()) {
      return;
    }

    this.page.update((value) => value + 1);
  }

  protected changePerPage(perPage: unknown): void {
    const nextPerPage = Number(perPage);

    if (
      !Number.isFinite(nextPerPage) ||
      nextPerPage <= 0 ||
      this.perPage() === nextPerPage
    ) {
      return;
    }

    this.perPage.set(nextPerPage);
    this.page.set(1);
  }

  protected closeForm(): void {
    this.showForm.set(false);
    this.editingId.set(null);
    this.detailRefreshTick.set(0);
    this.resetForm();
  }

  private reloadList(): void {
    this.refreshTick.update((value) => value + 1);
  }

  private patchForm(accountingGroup: AccountingGroup): void {
    this.form.reset({
      code: accountingGroup.code,
      accountClassId: accountingGroup.accountClassId,
      name: accountingGroup.name,
      description: accountingGroup.description,
      accountFrom: accountingGroup.accountFrom,
      accountTo: accountingGroup.accountTo,
      affectsClosing: accountingGroup.affectsClosing,
      affectsFinancialStatements: accountingGroup.affectsFinancialStatements
    });
  }

  private resetForm(): void {
    this.form.reset({
      code: '',
      accountClassId: null,
      name: '',
      description: '',
      accountFrom: null,
      accountTo: null,
      affectsClosing: false,
      affectsFinancialStatements: false
    });
  }

  private buildPayload(): AccountingGroupPayload {
    const value = this.form.getRawValue();

    return {
      code: value.code?.trim() ?? '',
      accountClassId: value.accountClassId ?? 0,
      name: value.name?.trim() ?? '',
      description: value.description?.trim() ?? '',
      accountFrom: value.accountFrom ?? 0,
      accountTo: value.accountTo ?? 0,
      affectsClosing: value.affectsClosing ?? false,
      affectsFinancialStatements: value.affectsFinancialStatements ?? false
    };
  }

  private extractErrorMessage(error: unknown): string {
    if (error instanceof HttpErrorResponse) {
      const apiMessage =
        (error.error?.message as string | undefined) ??
        (error.error?.error as string | undefined);

      return apiMessage ?? 'La API devolvio un error al procesar la solicitud.';
    }

    if (error instanceof Error) {
      return error.message;
    }

    return 'Ocurrio un error inesperado.';
  }

  private showToast(status: FeedbackStatus, title: string, message: string): void {
    this.toastrService.show(message, title, {
      status,
      duration: 4000,
      destroyByClick: true,
      hasIcon: true
    });
  }

  private mapAccountingGroupToTreeNode(
    accountingGroup: AccountingGroup
  ): TreeNode<AccountingGroupGridRow> {
    return {
      data: {
        id: accountingGroup.id,
        code: accountingGroup.code,
        name: accountingGroup.name,
        description: accountingGroup.description,
        accountClassName:
          accountingGroup.accountClass?.name ?? `Id ${accountingGroup.accountClassId}`,
        accountFrom: accountingGroup.accountFrom,
        accountTo: accountingGroup.accountTo,
        affectsClosing: accountingGroup.affectsClosing,
        affectsFinancialStatements: accountingGroup.affectsFinancialStatements,
        createdAt: accountingGroup.createdAt,
        updatedAt: accountingGroup.updatedAt,
        deletedAt: accountingGroup.deletedAt
      }
    };
  }
}
