...
type AccountingDocumentGridRow = {
  id: number;
  code: string;
  name: string;
  description: string;
...
@Component({
  selector: 'app-documentos-contables-page',
  imports: [
    DatePipe,
    ReactiveFormsModule,
    NbButtonModule,
  ...
  templateUrl: './documentos-contables-page.component.html',
  styleUrl: './documentos-contables-page.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class DocumentosContablesPageComponent {
  ...
  private readonly treeGridDataSourceBuilder = inject(
    NbTreeGridDataSourceBuilder
  ) as NbTreeGridDataSourceBuilder<AccountingDocumentGridRow>;
  private readonly listAccountingDocumentsUseCase = inject(ListAccountingDocumentsUseCase);
  private readonly getAccountingDocumentDetailUseCase = inject(
    GetAccountingDocumentDetailUseCase
  );
  private readonly createAccountingDocumentUseCase = inject(
    CreateAccountingDocumentUseCase
  );
  private readonly updateAccountingDocumentUseCase = inject(
    UpdateAccountingDocumentUseCase
  );
  private readonly deleteAccountingDocumentUseCase = inject(
    DeleteAccountingDocumentUseCase
  );
  private readonly restoreAccountingDocumentUseCase = inject(
  ...
  protected readonly treeGridDataSource = signal<
    NbTreeGridDataSource<AccountingDocumentGridRow>
  >(this.treeGridDataSourceBuilder.create([] as TreeNode<AccountingDocumentGridRow>[]));
  ...
  protected readonly accountingDocumentsResource = rxResource<
    AccountingDocumentPage,
  ...
  >({
    ...
    stream: ({ params }) =>
      this.listAccountingDocumentsUseCase.execute(params.page, params.perPage)
  });

  protected readonly selectedAccountingDocumentResource = rxResource<
  ...
  >({
    ...
    stream: ({ params }) =>
      params.id ? this.getAccountingDocumentDetailUseCase.execute(params.id) : of(null),
    defaultValue: null
  });

  protected readonly accountingDocumentPage = computed<AccountingDocumentPage | null>(
    () => this.accountingDocumentsResource.value() ?? null
  );
  protected readonly accountingDocuments = computed<AccountingDocument[]>(
    () => this.accountingDocumentPage()?.items ?? []
  );
  protected readonly pagination = computed(() => {
    const page = this.accountingDocumentPage();
  ...
  protected readonly formTitle = computed(() =>
    this.isEditing() ? 'Editar documento contable' : 'Nuevo documento contable'
  );
  ...
  constructor() {
    effect(() => {
      const accountingDocument = this.selectedAccountingDocumentResource.value();

      if (!accountingDocument) {
        return;
      }

      this.patchForm(accountingDocument);
    });

    effect(() => {
      const nodes = this.accountingDocuments().map((accountingDocument) =>
        this.mapAccountingDocumentToTreeNode(accountingDocument)
      );
  ...
  protected editAccountingDocument(id: number): void {
    this.showForm.set(true);
    this.showToast(
      'info',
      'Cargando detalle',
      'Se esta cargando la informacion completa del documento contable seleccionado.'
    );
  ...
  protected async submit(): Promise<void> {
    if (this.form.invalid) {
      ...
      this.showToast(
        ...
        'Revisa los campos obligatorios antes de guardar el documento contable.'
      );
      return;
    }
    ...
    try {
      if (this.isEditing() && this.editingId()) {
        await firstValueFrom(
          this.updateAccountingDocumentUseCase.execute(this.editingId()!, payload)
        );
        this.showToast(
          'success',
          'Registro actualizado',
          'El documento contable se actualizo correctamente.'
        );
      } else {
        await firstValueFrom(this.createAccountingDocumentUseCase.execute(payload));
        this.showToast(
          'success',
          'Registro creado',
          'El documento contable se creo correctamente.'
        );
      }
  ...
  protected async deleteAccountingDocument(id: number): Promise<void> {
    const confirmed = confirm(
      'Deseas eliminar este documento contable? Esta accion no se puede deshacer.'
    );
    ...
    try {
      await firstValueFrom(this.deleteAccountingDocumentUseCase.execute(id));
      this.showToast(
        'success',
        'Registro eliminado',
        'El documento contable fue eliminado correctamente.'
      );
  ...
  protected async restoreAccountingDocument(id: number): Promise<void> {
    const confirmed = confirm('Deseas restaurar este documento contable eliminado?');
    ...
    try {
      await firstValueFrom(this.restoreAccountingDocumentUseCase.execute(id));
      this.showToast(
        'success',
        'Registro restaurado',
        'El documento contable fue restaurado correctamente.'
      );
      this.reloadList();
    } catch (error) {
      this.showToast(
  ...
  protected refreshData(): void {
    ...
    this.showToast(
      'info',
      'Datos actualizados',
      'Se recargo la informacion de documentos contables desde la API.'
    );
  }
  ...
  private patchForm(accountingDocument: AccountingDocument): void {
    this.form.reset({
      code: accountingDocument.code,
      name: accountingDocument.name,
  ...
  private buildPayload(): AccountingDocumentPayload {
  ...
  private mapAccountingDocumentToTreeNode(
    accountingDocument: AccountingDocument
  ): TreeNode<AccountingDocumentGridRow> {
    return {
      data: {
        ...
        name: accountingDocument.name,
        description: accountingDocument.description,
        createdAt: accountingDocument.createdAt,
        updatedAt: accountingDocument.updatedAt,
        deletedAt: accountingDocument.deletedAt
      }
    };
  }
}
