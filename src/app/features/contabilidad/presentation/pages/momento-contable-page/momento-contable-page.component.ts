...
@Component({
  selector: 'app-momento-contable-page',
  imports: [
    DatePipe,
    ReactiveFormsModule,
    NbButtonModule,
  ...
  templateUrl: './momento-contable-page.component.html',
  styleUrl: './momento-contable-page.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class MomentoContablePageComponent {
  ...
  private readonly treeGridDataSourceBuilder = inject(
    NbTreeGridDataSourceBuilder
  ) as NbTreeGridDataSourceBuilder<AccountingMomentGridRow>;
  private readonly listAccountingMomentsUseCase = inject(ListAccountingMomentsUseCase);
  private readonly getAccountingMomentDetailUseCase = inject(GetAccountingMomentDetailUseCase);
  private readonly createAccountingMomentUseCase = inject(CreateAccountingMomentUseCase);
  private readonly updateAccountingMomentUseCase = inject(UpdateAccountingMomentUseCase);
  private readonly deleteAccountingMomentUseCase = inject(DeleteAccountingMomentUseCase);
  private readonly restoreAccountingMomentUseCase = inject(RestoreAccountingMomentUseCase);
  ...
  protected readonly treeGridDataSource = signal<
    NbTreeGridDataSource<AccountingMomentGridRow>
  >(this.treeGridDataSourceBuilder.create([] as TreeNode<AccountingMomentGridRow>[]));
  ...
  protected readonly accountingMomentsResource = rxResource<
    AccountingMomentPage,
  ...
  >({
    ...
    stream: ({ params }) =>
      this.listAccountingMomentsUseCase.execute(params.page, params.perPage)
  });

  protected readonly selectedAccountingMomentResource = rxResource<
    AccountingMoment | null,
  ...
  >({
    ...
    stream: ({ params }) =>
      params.id ? this.getAccountingMomentDetailUseCase.execute(params.id) : of(null),
    defaultValue: null
  });

  protected readonly accountingMomentPage = computed<AccountingMomentPage | null>(
    () => this.accountingMomentsResource.value() ?? null
  );
  protected readonly accountingMoments = computed<AccountingMoment[]>(
    () => this.accountingMomentPage()?.items ?? []
  );
  protected readonly pagination = computed(() => {
    const page = this.accountingMomentPage();
  ...
  protected readonly formTitle = computed(() =>
    this.isEditing() ? 'Editar momento contable' : 'Nuevo momento contable'
  );
  ...
  constructor() {
    effect(() => {
      const accountingMoment = this.selectedAccountingMomentResource.value();

      if (!accountingMoment) {
        return;
      }

      this.patchForm(accountingMoment);
    });

    effect(() => {
      const nodes = this.accountingMoments().map((accountingMoment) =>
        this.mapAccountingMomentToTreeNode(accountingMoment)
      );
  ...
  protected editAccountingMoment(id: number): void {
    this.showForm.set(true);
    this.showToast(
      'info',
      'Cargando detalle',
      'Se esta cargando la informacion completa del momento contable seleccionado.'
    );
  ...
  protected async submit(): Promise<void> {
    if (this.form.invalid) {
      ...
      this.showToast(
        ...
        'Revisa los campos obligatorios antes de guardar el momento contable.'
      );
      return;
    }
    ...
    try {
      if (this.isEditing() && this.editingId()) {
        await firstValueFrom(
          this.updateAccountingMomentUseCase.execute(this.editingId()!, payload)
        );
        this.showToast(
          'success',
          'Registro actualizado',
          'El momento contable se actualizo correctamente.'
        );
      } else {
        await firstValueFrom(this.createAccountingMomentUseCase.execute(payload));
        this.showToast(
          'success',
          'Registro creado',
          'El momento contable se creo correctamente.'
        );
      }
  ...
  protected async deleteAccountingMoment(id: number): Promise<void> {
    const confirmed = confirm(
      'Deseas eliminar este momento contable? Esta accion no se puede deshacer.'
    );
    ...
    try {
      await firstValueFrom(this.deleteAccountingMomentUseCase.execute(id));
      this.showToast(
        'success',
        'Registro eliminado',
        'El momento contable fue eliminado correctamente.'
      );
  ...
  protected async restoreAccountingMoment(id: number): Promise<void> {
    const confirmed = confirm('Deseas restaurar este momento contable eliminado?');
    ...
    try {
      await firstValueFrom(this.restoreAccountingMomentUseCase.execute(id));
      this.showToast(
        'success',
        'Registro restaurado',
        'El momento contable fue restaurado correctamente.'
      );
      this.reloadList();
    } catch (error) {
      this.showToast(
  ...
  protected refreshData(): void {
    ...
    this.showToast(
      'info',
      'Datos actualizados',
      'Se recargo la informacion de momentos contables desde la API.'
    );
  }
  ...
  private patchForm(accountingMoment: AccountingMoment): void {
    this.form.reset({
      code: accountingMoment.code,
      name: accountingMoment.name,
      description: accountingMoment.description
    });
  }
  ...
  private buildPayload(): AccountingMomentPayload {
  ...
  private mapAccountingMomentToTreeNode(
    accountingMoment: AccountingMoment
  ): TreeNode<AccountingMomentGridRow> {
    return {
      data: {
        id: accountingMoment.id,
        code: accountingMoment.code,
        name: accountingMoment.name,
        description: accountingMoment.description,
        createdAt: accountingMoment.createdAt,
        updatedAt: accountingMoment.updatedAt,
        deletedAt: accountingMoment.deletedAt
      }
    };
  }
}
